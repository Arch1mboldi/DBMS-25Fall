...
for filename in list(os.listdir('.')):
    if filename.endswith('.sql'):
        ...
        if not os.system('diff -ZB ans/%s.ans %s.ans' % (f0, f0)):
            ...
            print('[Fail] %s, elapsed = %.2lfs' % (filename, time.time() - t_start))
            break
        os.system('rm *.database *.thead *.tdata %s.sql %s.ans ans/%s.ans' % (f0, f0, f0))
